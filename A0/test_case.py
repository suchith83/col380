...
if __name__ == "__main__":
    MATRIX_SIZES = [3000, 4000, 5000]
    LOOP_TYPES = [0, 1, 2, 3, 4, 5]
    # writer = csv.writer(open("data_1.csv", "w"))
    # writer.writerow(["size", "loop_type", "time"])
    with open('data_1.csv', 'a', newline='') as f:
        ...
        for size in MATRIX_SIZES:
            ...
            for loop_type in LOOP_TYPES:
                print("Running test for type {} with size {}x{}...".format(loop_type, size, size))
                result, time_duration = execute_test_case(loop_type, rows, cols, inner, "./input_path/", "./output_path/")
                writer.writerow([size, loop_type, time_duration])
                if result:
                    print("Test Case passed")
                else:
                    print("Test Case failed")

    # result, time_duration = execute_test_case(0, 1000, 1000, 1000, "./input_path/", "./output_path/")
    # if result:
    #     print("Test Case passed")
    # else:
    #     print("Test Case failed")